...
- **Gemini** (AI Studio) — a free API key, but Google's free-tier keys have
  ...
  testing; see the project's own notes if you hit a persistent `403`.
...
`scripts/deploy.py`'s `config` check reports it as a warning, not a
blocker. See [Model pricing](../reference/pricing.md) for the models this
project has verified rates for.

## Next

Steps 1–4 are done. Continue to `local/05-postgres.md` (Local track) or
`hosted/05-supabase.md` (Hosted track) — see the [setup overview](index.md)
