...
Expected: FAIL with `AttributeError: module 'bot.scripts.deploy' has no attribute 'CHECKS'`.
...
Expected: FAIL with `ModuleNotFoundError: No module named 'bot.scripts.gen_docs'`.
...
**Interfaces:**
- Consumes: Task 2's `GENERATED_HEADER`; `app.providers.pricing._RATES` and `Rate`; `app.providers.registry.PROVIDERS`; `bot.scripts.deploy`'s `_ALWAYS_SYNCED`, `_GENERIC_OPERATIONAL_ENV_ATTRS`, `_DB_SYNCED_OPERATIONAL_KEYS`, `_NEVER_SYNCED_OPERATIONAL_KEYS`.
...
Expected: FAIL with `AttributeError: module 'bot.scripts.gen_docs' has no attribute 'render_pricing'`.
    ...
    return "\n".join(
        [
            ...
            *bullets(db_only),
            ...
            "redeploy. `uv run python -m bot.scripts.deploy --sync-config-db` pushes "
...
Expected: FAIL with `AttributeError: module 'bot.scripts.gen_docs' has no attribute 'render_checks'`.
...
def render_checks() -> str:
    ...
    lines = [
        GENERATED_HEADER,
        "# Deployment checks\n",
        "`uv run python -m bot.scripts.deploy` runs all of these and prints one line "
...
Expected: FAIL with `AttributeError: module 'bot.scripts.gen_docs' has no attribute 'write_all'`.
...
**Interfaces:**
- Consumes: `bot.scripts.gen_docs` (Task 5).
